...
def custom_score(game, player):
    ...
    blanks_left = len(game.get_blank_spaces())
    
    w, h = game.width, game.height
    y, x = game.get_player_location(player)
    
    distance = float((h - y)**2 + (w - x)**2) / ((1/2 * w)**2 + (1/2 * h)**2)
   
    total_space = w * h
    
    ratio = blanks_left / total_space
    flipping_minus_plus = (1/2 * total_space - blanks_left) / total_space

    return float(own_moves - opp_moves + distance * flipping_minus_plus) # float(own_moves - opp_moves)
...
def custom_score_2(game, player):
    ...
    return float(own_moves - ratio * opp_moves)
    #return float(1.5 * own_moves - opp_moves)
...
def custom_score_3(game, player):
    ...
    return float(own_moves - ratio * opp_moves + distance * flipping_minus_plus) # float(own_moves - opp_moves)
...
class MinimaxPlayer(IsolationPlayer):
        ...
        **********************************************************************
        ...
        for m in game.get_legal_moves():
            ...
            if v > best_score:
                ...
                best_move = m
        """
        if game._active_player == game._player_1:
            best_score = float("-inf")
            for m in game.get_legal_moves():
                v = self.min_value(game.forecast_move(m), depth - 1)
                if v > best_score:
        ...
        if game._active_player == game._player_2:
            best_score = float("inf")
            for m in game.get_legal_moves():
    ...
    def min_value(self, game, depth):
        ...
        if depth <= 0 or not bool(game.get_legal_moves()):
            return self.score(game, self)  # game._active_player
